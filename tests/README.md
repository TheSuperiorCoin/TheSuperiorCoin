...
Core tests take longer than any other Superior tests, due to the high amount of computational work involved in validating core components.
...
To run only Superior's core tests (after building):
...
To run only Superior's crypto tests (after building):
...
```bash
superiord --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `superior-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Superior's hash tests (after building):
...
To run only Superior's performance tests (after building):
...
To run only Superior's unit tests (after building):
