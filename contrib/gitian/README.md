...
*Setup instructions for a Gitian build of Superior.*

Gitian is the deterministic build process that is used to build the Superior CLI
...
```bash
git clone git@github.com:superior-project/gitian.sigs.git
...
gpg --output $VERSION-linux/$NAME/superior-linux-$VERSION-build.assert.sig --detach-sign $VERSION-linux/$NAME/superior-linux-$VERSION-build.assert
gpg --output $VERSION-osx-unsigned/$NAME/superior-osx-$VERSION-build.assert.sig --detach-sign $VERSION-osx-unsigned/$NAME/superior-osx-$VERSION-build.assert
gpg --output $VERSION-win-unsigned/$NAME/superior-win-$VERSION-build.assert.sig --detach-sign $VERSION-win-unsigned/$NAME/superior-win-$VERSION-build.assert
```

Make a pull request (both the `.assert` and `.assert.sig` files) to the
[superior-project/gitian.sigs](https://github.com/superior-project/gitian.sigs/) repository:
...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/superior-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/superior-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/superior-osx-*-build.assert
```
...
```bash
./gitian-build.py --detach-sign --no-commit --url https://github.com/superiormooo-superior/bitsuperior -b superiormooo 1f5680c8db8f4cc7acc04a04c724b832003440fd
```
